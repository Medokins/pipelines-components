...
Reads the platform-injected ``KFP_MLFLOW_CONFIG`` blob (Kubeflow/RHOAI native MLflow integration); no custom connection secret is required. When that env var is absent, logging is skipped and the step completes successfully.
...
| `pipeline_name` | `str` | `None` | Stable pipeline name used only for run tags. Pass the pipeline's logical name (e.g. ``PIPELINE_NAME``). The MLflow experiment and parent run come from ``KFP_MLFLOW_CONFIG``. |
...
| `run_name` | `str` | `""` | Per-execution run name recorded as a tag on child runs. Pass the run's display name (from ``dsl.PIPELINE_JOB_NAME_PLACEHOLDER``). Falls back to ``pipeline_name`` when empty. |
